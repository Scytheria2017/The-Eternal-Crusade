...
{
    float value = std::max(0.f, (m_auraBaseFlatMod[SHIELD_BLOCK_VALUE] + GetStat(STAT_STRENGTH)) * m_auraBasePctMod[SHIELD_BLOCK_VALUE]);
    return uint32(value);
}
...
void Player::UpdateCombatSkills(Unit* victim, WeaponAttackType attType, bool defense)
{
    ...
    float chance = (float(3 * lvldif * skilldif) / plevel) / 10.0f;

    chance = chance < 0.1f ? 0.1f : chance;                 //minimum chance to increase skill is 0.1%
